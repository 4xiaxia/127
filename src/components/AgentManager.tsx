...
const AgentManager: React.FC = () => {
  ...
  // 模拟智能处理
  const simulateIntelligentProcessing = async (query: string): Promise<{
    answer: string;
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        // 模拟小抄优先策略
        if (query.includes('郑玉指') || query.includes('红色') || query.includes('景点')) {
          resolve({
      ...
      }, Math.random() * 2000 + 500); // 0.5-2.5秒随机处理时间
    });
  };
    ...
    <div style={{ 
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ marginBottom: '12px' }}>
            <strong>模式：</strong> {agentA.mode === 'standby' ? '🔋 待机' : 
                              agentA.mode === 'eye_mode' ? '📝 眼睛模式' : '🎤 耳朵模式'}
          </div>
          <div style={{ marginBottom: '12px' }}>
            <strong>监听状态：</strong> {agentA.isListening ? '🟢 活跃' : '🔴 静默'}
          </div>
          <div style={{ marginBottom: '12px' }}>
        ...
        <div style={{
          ...
          <div style={{ marginBottom: '12px' }}>
            <strong>处理状态：</strong> {agentB.isProcessing ? '🔄 处理中' : '✅ 空闲'}
          </div>
          <div style={{ marginBottom: '12px' }}>
            <strong>查询来源：</strong> {agentB.querySource === 'cheat_sheet' ? '📚 小抄' :
                                   agentB.querySource === 'ai_model' ? '🤖 AI模型' : '🔍 MCP搜索'}
          </div>
          <div style={{ marginBottom: '12px' }}>
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '20px' }}>
          <div style={{ marginBottom: '12px', fontWeight: 'bold' }}>选择输入模式：</div>
        ...
        {agentA.mode === 'eye_mode' && (
          <div style={{ marginBottom: '20px' }}>
            <div style={{ marginBottom: '8px', fontWeight: 'bold' }}>文字输入：</div>
            <input
              type="text"
              value={textInput}
        ...
        {agentA.mode === 'ear_mode' && (
          <div style={{ marginBottom: '20px' }}>
            <div style={{ marginBottom: '8px', fontWeight: 'bold' }}>语音输入：</div>
            <button
